...
- 修复导入包含多个面的 KML、KMZ 或 GeoJSON 区域文件时仅按第一个面下载、严格裁剪不完整的问题；现在会显示、枚举、下载并精确裁剪所有外环，同时兼容旧任务的单面范围数据
- 修复多个面区域的恢复下载仅裁剪本次新增瓦片、遗漏已有瓦片的问题；恢复完成后会统一扫描并严格裁剪全部瓦片
- 修复地图实例被 Vue 深度代理，未选中图层时缩放地图仍明显卡顿的问题

### 优化

- 多面区域下载按所有外环的并集枚举瓦片，避免仅下载第一个面或遗漏相互分离的区域
- 多面严格裁剪使用扫描线区间并集处理，支持分离、相邻和重叠面，减少重复像素处理
- 增加多边形边界与瓦片相交检测，避免凹多边形缺口被误判为完整瓦片而跳过裁剪
- 首次栅格任务可在下载过程中并发执行边界瓦片裁剪；恢复任务自动切换为全量裁剪，保证已有瓦片与新增瓦片结果一致
- 增加旧版单面区域数据格式兼容解析，升级后无需重新创建已有任务

### 测试

- 增加多面区域解析、瓦片枚举、WebMercator 裁剪、断开区域裁剪、凹多边形缺口和恢复下载裁剪路径测试
- 已验证前端 TypeScript 类型检查、Rust 多面单元测试和后端编译检查
...
- 修复更新器返回的 Tauri `Update` 资源句柄被 Vue 深度代理后，安装更新时报 `Cannot read private member` 并中断的问题
...
- 修复浏览器可访问 Google 等瓦片源、但 `fetch_tile` 与下载引擎因未继承 Windows/macOS 系统代理而连接失败的问题
- 为自定义代理补充 SOCKS5 支持，并让本机回环瓦片源始终直连，避免系统代理干扰本地发布预览
- 自动清理从 Markdown / HTML 复制的 `\&`、`&amp;` URL 转义符
- 新建 TMS/WMTS/网页抓取任务时可在「请求配置」中明确设置 64–4096 像素的瓦片大小，不再根据 URL 参数自动猜测；Google `scale=2` 可手动设置为 512
- 修复已下载的 512 像素瓦片仍按 256 像素加载，导致任务主地图和借数据缩略预览出现层级错位的问题
- GCJ02 在线预览与下载后纠偏改为使用任务配置的实际瓦片尺寸，并支持解析 Google `/vt/lyrs=s&x=...` 旧式无问号参数地址
- 请求配置新增 WGS84/GCJ02 坐标偏移类型；中国区偏移图源可明确启用 GCJ02 纠偏
- 借数据候选与后端导入校验增加 URL、坐标类型和瓦片尺寸一致性检查，避免混用 256/512 瓦片
- 修复旧图层中的 `Connection`、`Accept-Encoding` 等浏览器请求头导致新版代理拒绝整个瓦片请求的问题；相关逐跳请求头现在会安全过滤并记录日志
- 修复旧图层使用默认全球范围时被强制缩放到图源最小层级以下，造成已保存图层看似全部空白的问题；GCJ02 全球图源会保留或恢复中国区域视点
- MBTiles 导出写入实际 `tile_size` 与 HiDPI `tilepixelratio` 扩展元数据，重新导入 512 像素 Google 瓦片时不再回退到 256 导致显示错位；未声明尺寸的外部文件仍默认 256

### 优化

- 瓦片代理补充浏览器 User-Agent、压缩协商和代理模式日志；无效代理地址不再静默回退为直连
...
- **H 套件（H1 + H2 + H3 + H4）— 矢量瓦片基础支持**：下载链接含 `pbf` / `mvt` / `vector` 关键字时自动识别为矢量格式 (format=pbf)
  ...
  - **PMTiles 导出**（H4）：矢量任务使用 `TileType::Mvt` + `Compression::Gzip`（由 PMTiles writer 自动 gzip 包装，无需手动）；防御式 `maybe_gunzip` 处理存储侧偶然双重压缩；metadata JSON 同样写入 `vector_layers` 描述
  - **依赖**：新增 `@mapbox/vector-tile` + `pbf` 两个前端依赖用于 MVT 内省；后端新增轻量 `mvt_scan` 模块（纯 Rust，无 protobuf crate 依赖）
  - **暂未支持**（H5 后续）：GeoJSON 导出
