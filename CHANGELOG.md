...
- **GeoTIFF 压缩导出**：导出 GeoTIFF 时可选择压缩方式（无压缩 / LZW 无损 / DEFLATE 无损），默认无压缩；通过 vendor 本地修补 `tiff` 0.9.1 库，修复 `write_strip` 不激活压缩器的底层缺陷，确保 LZW / DEFLATE 条带均可被 GDAL / QGIS 正确读取
